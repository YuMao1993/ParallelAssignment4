...
/*
 * The worker node contains two, six-core Xeon e5-2620 v3 processors (2.4 
 * GHz, 15MB L3 cache, hyper-threading, AVX2 instruction support) thus the 
 * total working context number is 2 * 6 * 2 = 24
 */
...
// Implements logic required by compareprimes command
// via multiple
// calls to execute_work.  This function fills in the
// appropriate
// response.
static void
execute_compareprimes(const Request_msg& req, Response_msg& resp)
{
...
workerThread(void* args)
{
    while (true) {
        ...
        if (req.get_arg("cmd").compare("compareprimes") == 0) {
            ...
            // The compareprimes command needs to be special cased since
            // it is
            ...
            // actually perform the work.  The response string is filled
            // in by
            // 'execute_work'
...
static void*
projectIdeaWorkerThread(void* args)
{
    while (true) {
        Request_msg req;
